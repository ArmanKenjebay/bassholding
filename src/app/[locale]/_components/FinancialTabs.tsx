...
export default function FinancialTabs() {
  ...
  return (
    <div className={`flex flex-col`}>
      <span
        className={`text-primary-gold xl:text-[48px] lg:text-[32px] md:text-[28px] sm:text-[24px] text-[21px]`}
      >
        ...
        {years.map((year, index) => (
          <Tab key={year.year} title={year.year}>
            <div className={`flex flex-wrap gap-7`}>
              {year.parts.map((part) => (
                <div
                  className={`w-[300px] h-[235px] rounded-xl flex flex-col justify-between bg-[#262626] p-5`}
                >
